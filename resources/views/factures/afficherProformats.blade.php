...
<head>
    ...
    <title>{{ $factures->numeroFacture }} - {{$proformats->numeroClient ?? $proformats->clients->numero?? 'FACTURE VENTE'}}</title>
...
<body>
    <div class="invoice-container">
        ...
        <div class="ref-proforma">
            ...
            <h2 class="proforma-title">FACTURE proformats</h2>
            <div class="agent-operant">Agent opérant: <strong>{{ $proformats->user->name ?? $proformats->agentOperant?? '-'}}</strong> </div>
            <div class="agent-operant" style="margin-top: 1em;">Date: <strong>{{ $proformats->created_at->format('d-m-y')}}</strong> </div>
        </div>
        ...
        <section class="client-info">
            ...
            <div class="client-details-row">
                <div class="client-nom">
                    <p>NOM du client : <strong>{{ $proformats->clients->nom?? $proformats->commandes->clients->nom?? '-'}} </strong> </p>
                </div>
                <div class="client-contact">
                    <p>Contacts : <strong> {{$proformats->clients->numero?? $proformats->commandes->clients->numero?? '-'}}</strong></p>
                </div>
                <div class="client-adresse">
                    <p>Adresse : <strong>{{$proformats->clients->adresse?? $proformats->commandes->clients->adresse?? '-'}}</strong> </p>
                </div>
            </div>
        </section>
        ...
        <div class="content">
            <div class="services">
                <table style=" width: 40em;   border-style: solid; border-color: black;">
                    ...
                    <tr >
                        ...
                        <td><strong>{{ $proformats->mainOeuvre }}</strong></td>
                    </tr>
            ...
            <div class="mini-footer" style="text-align:center">
                @if($proformats->reduction > 0)
                    <div class="total"> 
                        <span style="margin-bottom: 3em; Z-index: 5; background-color:grey; color: black !important"> la reduction sur votre facture est de : <strong>{{ $proformats->reduction}} Francs CFA</strong> </span>
                    </div>
                @endif
                <div class="total"> 
                    <span style="margin-bottom: 3em; Z-index: 5; background-color:grey; color: black !important; margin-left: 25em;"> Net A Payer <strong style="color: #27ae60; background-color: #e9f7ef;"> <u>{{ $proformats->NetAPayer}} Francs CFA</u> </strong> </span>
                </div>
