...
<html lang="fr">
    ...
    <body>
        ...
        <div id="content" style="margin-top: -3px;" >
            <div class="client">
                <p>Nom du client: <strong style="background-color: white;">{{$recus->clients->nom}}</strong></p>
                <p>Numero du client: <strong style="background-color: white;">{{$recus->clients->numero}}</strong></p>
                <p>Motif: <strong style="background-color: white;">{{$recus->remarque}}</strong></p>
                <p>Date limite du prochain versement: <strong style="background-color: white;">{{ \Carbon\Carbon::parse($recus->dateLimitePaiement)->format('d-m-Y') }}</strong></p>
            </div>
            <div id="montant"  >
                <p><strong>Montant versé:</strong> <strong style="background-color: white; margin-left: 4em;">{{$recus->montant_recu}} </strong>F CFA</p>
                <p><strong>Dette Precedente:</strong>  <strong style="background-color: white; margin-left: 3em;">{{$recus->montant_recu + $recus->reste}} </strong>F CFA</p>
                <p><strong>Dette Restante:</strong> <strong style="background-color: white; margin-left: 4em;">{{$recus->reste}} </strong>F CFA</p>
            </div>
        </div>
        <hr>
